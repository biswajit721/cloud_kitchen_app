...
import { ShoppingCart, X, Trash2, Plus, Minus, Menu, ChefHat, Home, Info, Phone, UtensilsCrossed, ClipboardList, Settings } from "lucide-react";
...
const NavStyles = () => (
  <style>{`
    ...
    * { box-sizing: border-box; }
    ...
    .nb-btn-solid:hover { background: var(--orange-dark); border-color: var(--orange-dark); transform: translateY(-1px); box-shadow: 0 6px 20px rgba(255,122,0,0.36); }
    ...
    .nb-drawer-link .drawer-icon { color: inherit; flex-shrink: 0; }

    /* ── Cart sidebar ── */
    .nb-cart-overlay {
      position: fixed; inset: 0; background: rgba(0,0,0,0.50);
      z-index: 970; backdrop-filter: blur(6px);
      animation: fadeIn 0.2s ease;
    }
    .nb-cart-sidebar {
        ...
        <div style={{
          ...
          <Link to="/" style={{
            ...
            <div style={{
              ...
              boxShadow: "0 4px 12px rgba(255,122,0,0.30)",
              fontSize: 18,
            }}>
              🍔
            </div>
          ...
          {/* ── Desktop Nav Links ── */}
          <div className="nb-desktop-links" style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
            ...
            {/* Cart Button */}
            <button className="nb-cart-btn" onClick={() => setIsCartOpen(true)}
              title="Open cart"
            >
            ...
            <div className="nb-desktop-links" style={{ display:"flex", alignItems:"center", gap:10 }}>
              {user ? (
                <>
                  ...
                  {user.role !== "admin" && (
                    <Link to="/my-orders" className={`nb-btn-ghost${isActive("/my-orders") ? " active" : ""}`}
                  ...
                  {user.role === "admin" && (
                    <Link to="/admin" className={`nb-btn-ghost${isActive("/admin") ? " active" : ""}`}
                  ...
                  <button className="nb-btn-logout" onClick={logout}>Logout</button>
                </>
              ) : (
                <>
            ...
            <button className="nb-hamburger" onClick={() => setIsMobileOpen(true)}>
          ...
          <div className="nb-mobile-drawer">
            ...
            {user && (
              <div style={{ margin:"14px 10px", padding:"14px 16px", background:"var(--green-light)", borderRadius:14, border:"1px solid rgba(46,204,113,0.25)", display:"flex", alignItems:"center", gap:12 }}>
                <div style={{ width:40, height:40, background:"var(--green)", borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center", fontSize:14, fontWeight:900, color:"#fff", flexShrink:0 }}>
                  {initials}
                </div>
                <div>
                  <div style={{ fontSize:14, fontWeight:700, color:"var(--text)" }}>{user.name}</div>
                  <div style={{ fontSize:11, color:"var(--green)", fontWeight:600 }}>
                    {user.role === "admin" ? "🛡️ Administrator" : "✅ Active Member"}
                  </div>
                </div>
              </div>
            )}
            ...
            <div style={{ padding:"8px 0" }}>
              <div style={{ padding:"6px 20px 8px", fontSize:10, fontWeight:800, color:"var(--text-light)", letterSpacing:"0.10em", textTransform:"uppercase" }}>
                Navigation
              </div>
              ...
              {user && user.role !== "admin" && (
                <Link to="/my-orders" className={`nb-drawer-link${isActive("/my-orders") ? " active" : ""}`}>
                  <ClipboardList size={16} className="drawer-icon"/> My Orders
                </Link>
              )}
              {user && user.role === "admin" && (
                <Link to="/admin" className={`nb-drawer-link${isActive("/admin") ? " active" : ""}`}>
                  <Settings size={16} className="drawer-icon"/> Admin Panel
                </Link>
              )}
            </div>
              ...
              {user ? (
                ...
                <>
                  <Link to="/login" style={{
                    ...
                    background:"var(--cream)", border:"1.5px solid var(--border)",
                    color:"var(--text-mid)", borderRadius:12,
                  ...
                  <Link to="/register" style={{
                    ...
                    boxShadow:"0 4px 16px rgba(255,122,0,0.30)",
                  }}>
                    Get Started Free →
                  </Link>
                </>
          ...
          <div className="nb-cart-sidebar">
            ...
            <div style={{
              ...
              <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                ...
                <div>
                  <h2 style={{ fontFamily:"'Sora',sans-serif", fontSize:16, fontWeight:900, color:"var(--text)", margin:0 }}>
                    Your Cart
                  </h2>
                  <p style={{ fontFamily:"'Nunito',sans-serif", fontSize:12, color:"var(--text-light)", margin:0 }}>
            ...
            <div style={{ flex:1, overflowY:"auto", padding:"0 20px" }}>
              {cartItems.length === 0 ? (
                <div style={{ textAlign:"center", padding:"60px 20px" }}>
                  ...
                  <h3 style={{ fontFamily:"'Sora',sans-serif", fontSize:17, fontWeight:800, color:"var(--text)", marginBottom:8 }}>
                    Your cart is empty
                  </h3>
                  <p style={{ fontFamily:"'Nunito',sans-serif", fontSize:14, color:"var(--text-light)", marginBottom:24 }}>
                    Add some delicious food to get started!
                  </p>
                  <button onClick={() => { setIsCartOpen(false); navigate("/foods"); }}
                    style={{
                      background:"var(--orange)", color:"#fff", border:"none",
                      borderRadius:12, padding:"11px 28px",
                      fontFamily:"'Sora',sans-serif", fontSize:13, fontWeight:800,
                      cursor:"pointer", boxShadow:"0 4px 16px rgba(255,122,0,0.28)",
                    }}
                  >
                    Browse Menu →
                  </button>
                ...
                cartItems.map(item => (
                  <div key={item._id} className="cart-item-row">
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      ...
                      {/* Qty controls */}
                      <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                        <button className="qty-btn minus" onClick={() => updateQuantity(item._id, item.quantity - 1)}>
                          <Minus size={11} color="var(--text-mid)"/>
                        </button>
                        <span style={{ fontFamily:"'Sora',sans-serif", fontSize:13, fontWeight:800, color:"var(--text)", minWidth:20, textAlign:"center" }}>
                          {item.quantity}
                        </span>
            ...
            {cartItems.length > 0 && (
              <div style={{
                ...
                <div style={{
                  display:"flex", alignItems:"center", gap:8,
                  background:"var(--yellow-light)", border:"1px solid rgba(255,195,0,0.40)",
                  borderRadius:10, padding:"8px 12px", marginBottom:14,
                }}>
                  <span style={{ fontSize:14 }}>🎉</span>
                  <span style={{ fontFamily:"'Nunito',sans-serif", fontSize:12, fontWeight:700, color:"#92670A" }}>
                    Free delivery applied! Save ₹49
                  </span>
                </div>
