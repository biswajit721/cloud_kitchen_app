                ...
                <div key={order._id} className="ao-card" style={{ animationDelay:`${Math.min(idx*.04,.3)}s` }}>
                  ...
                  <div style={{ padding:"16px 18px 14px" }}>
                    <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:10 }}>
                      <div>
                        <div style={{ fontSize:11.5, fontWeight:700, color:"#9CA3AF", letterSpacing:"0.05em", textTransform:"uppercase", marginBottom:3 }}>
                    ...
                    <div style={{ display:"flex", gap:14, flexWrap:"wrap", marginBottom:12 }}>
                      ...
                      <span style={{ display:"flex", alignItems:"center", gap:4, fontSize:11.5, color:"#5C5C6E", fontFamily:"'Nunito',sans-serif", fontWeight:600 }}>
                        <CreditCard size={11} color="#2ECC71"/> {order.paymentStatus}
                      </span>
                    </div>

                    {/* Amount */}
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
                      <span style={{ fontSize:12, color:"#9CA3AF", fontFamily:"'Nunito',sans-serif" }}>{order.items?.length || 0} item{(order.items?.length||0)!==1?"s":""}</span>
                      <span style={{ fontSize:18, fontWeight:900, color:"#FF7A00" }}>₹{order.totalAmount}</span>
                    </div>

                    {/* Expand items */}
                    <button type="button" onClick={() => toggle(order._id)}
                      style={{ width:"100%", background:"#FFF9F2", border:"1.5px solid #F0ECE6", borderRadius:10, padding:"7px", fontFamily:"'Sora',sans-serif", fontSize:12, fontWeight:700, color:"#FF7A00", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:5, marginBottom:12, transition:"background .15s" }}
                    ...
                    {/* Items */}
                    {isOpen && (
                      <div style={{ background:"#FFF9F2", borderRadius:12, padding:"10px 12px", marginBottom:12, maxHeight:160, overflowY:"auto" }}>
                        {order.items?.map((item, i) => (
                          <div key={i} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"5px 0", borderBottom: i < order.items.length-1 ? "1px solid #F0ECE6":"none" }}>
                            <span style={{ fontSize:12.5, color:"#2C2C2C", fontFamily:"'Nunito',sans-serif", fontWeight:600 }}>
                              {item.food?.name} × {item.quantity}
                            </span>
                            <span style={{ fontSize:12.5, fontWeight:800, color:"#FF7A00" }}>₹{(item.food?.price||0)*item.quantity}</span>
                          </div>
                        ))}
                      </div>
                    )}
                    ...
                    <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                      <div style={{ position:"relative" }}>
                        <label style={{ fontSize:10, fontWeight:700, color:"#9CA3AF", textTransform:"uppercase", letterSpacing:"0.05em", display:"block", marginBottom:4 }}>Order Status</label>
                        <select className="ao-select" value={order.orderStatus}
                          onChange={e => handleStatusChange(order._id, "orderStatus", e.target.value)}
                          disabled={!!updating[order._id+"orderStatus"]}
                          style={{ opacity: updating[order._id+"orderStatus"] ? .6 : 1 }}
                        >
                          {orderStatusOptions.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                        </select>
                      </div>
                      <div>
                        <label style={{ fontSize:10, fontWeight:700, color:"#9CA3AF", textTransform:"uppercase", letterSpacing:"0.05em", display:"block", marginBottom:4 }}>Payment Status</label>
                        <select className="ao-select" value={order.paymentStatus}
                          onChange={e => handleStatusChange(order._id, "paymentStatus", e.target.value)}
                          disabled={!!updating[order._id+"paymentStatus"]}
                          style={{ opacity: updating[order._id+"paymentStatus"] ? .6 : 1 }}
                        >
